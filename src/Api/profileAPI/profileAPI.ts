...
export const profileAPI = {
  getProfile(userId: string) {
    ...
    return instance.get<ProfileType>(endpoint).then(res => res.data);
  },
  getStatus(userId: string) {
    const endpoint = `profile/status/${userId}`;
    return instance.get<string>(endpoint).then(res => res.data);
  },
  updateStatus(status: string) {
    const endpoint = `profile/status`;
    return instance.put<APIResponseType>(endpoint, { status }).then(res => res.data);
  },
  savePhoto(photoFile: File) {
    ...
    return instance
      .put<APIResponseType<SavePhotoResponseDataType>>(endpoint, formData, {
        headers: {
